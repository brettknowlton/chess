...
pub struct Piece {
    ...
    pub targets: Vec<(usize, usize)>, //possible target squares
}
...
impl Piece {
    ...
    pub fn find_targets(self, board: &board::BoardState) -> Vec<(usize, usize)> {
            ...
            PieceType::Pawn => {
                ...
                if forward_rank < 8 {
                    if board.pieces.get(&(file, forward_rank)).is_none() {
                        ...
                        targets.push((file, forward_rank));
                    }
                    //initial double move
                        ...
                        || (self.color == PieceColor::Black && rank == 6)
                    {
                        ...
                        {
                            //only add forward move if square is empty
                            targets.push((file, double_forward_rank));
                        }
                    }
                }
                ...
                if file > 0 {
                    if let Some(seen_piece) = board.pieces.get(&(file - 1, forward_rank)) {
                        if seen_piece.color != self.color {
                            targets.push((file - 1, forward_rank));
                        }
                    }
                }
                if file < 8 {
            ...
            PieceType::Knight => {
                ...
                for km in knight_moves {
                    ...
                    if target_file >= 0 && target_file < 8 && target_rank >= 0 && target_rank < 8 {
                        if let Some(seen_piece) = board.pieces.get(&(target_file as usize, target_rank as usize)) {
                            if seen_piece.color != self.color {
                                targets.push((target_file as usize, target_rank as usize));
                            }
                        } else {
                            targets.push((target_file as usize, target_rank as usize));
                        }
